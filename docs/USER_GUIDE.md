...
3. Выберите `minesplat-prism-1.21.1-0.1.0.mrpack`.
...
Смена 32/64/128 в текущей сессии повторяет только вокселизацию существующего PLY.
Смена палитры или blacklist при том же разрешении пересобирает схему локально.
Смена seed или изображения создаёт новую Gaussian-модель.

### Ручная установка

1. В Prism нажмите `Add Instance → Custom`.
2. Выберите Minecraft `1.21.1` и Fabric Loader `0.16.2`.
...
4. Через `Add File` добавьте
   `minesplat-fabric-1.21.1-0.1.0.jar`.
...
3. Choose `minesplat-prism-1.21.1-0.1.0.mrpack`.
...
Create a Minecraft 1.21.1 instance with Fabric Loader 0.16.2. Install Fabric API
